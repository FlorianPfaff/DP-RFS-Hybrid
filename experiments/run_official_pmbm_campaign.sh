...
pids=()
csv_files=()
for ((seed_start = 0; seed_start < seed_count; seed_start += chunk_size)); do
    remaining=$((seed_count - seed_start))
...
"$python_bin" "$project_root/experiments/merge_metric_csv.py" \
    --input "${csv_files[@]}" \
    --output "$project_root/results/recurring_birth_pmbm_seed${seed_count}.csv"
